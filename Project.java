...
public class Project extends Application
{
   ...
   public void start(Stage stage)
   {
      ...
      for (int i = 0; i < 4; i++) {//Build the game board
         for (int j = 0; j < 4; j++) {
            gamePane = new GamePane(i,j);//add the i and j positions to each game pane
   ...
   public void click(String direction, int posI, int posJ){//click method changes which circles are visible based on the input direction
      ...
      if(direction.equals("Kicked Down") && posJ <=1 && board[posI][posJ + 1].getCircle() == true && board[posI][posJ + 2].getCircle() == false){//if the top button is pushed, this gathers the position data
      
         //System.out.println("i in click method= "+posI + " j= " + posJ);// kept outside of the for loop or all the circles in the row/column will change
      ...
      else if(direction.equals("Kicked Up") && posJ >1 && board[posI][posJ - 1].getCircle() == true && board[posI][posJ - 2].getCircle() == false){
      ...
      else if(direction.equals("Kicked Right") && posI <=1 && board[posI+1][posJ].getCircle() == true && board[posI+2][posJ].getCircle() == false){
      ...
      else if(direction.equals("Kicked Left") && posI >1 && board[posI-1][posJ].getCircle() == true && board[posI-2][posJ].getCircle() == false){
      ...
      if (!hasMovesLeft()&&counter>1) { // check if there are no more circles and no more moves
   ...
   public class GamePane extends GridPane {//creates the canvas and methods to toggle the buttons and circle visibility
      ...
      public GamePane(int iPos, int jPos) {// pass in the positions of the game pane when the array is built
         this.iPos=iPos;
         this.jPos=jPos;
         
         canvas = new Canvas(80, 80);//build the canvas
         add(canvas, 1, 1);
      
         topButton.setPrefSize(80, 20);// add the buttons
         ...
         topButton.setOnAction(new ButtonListener());// set the buttons to action in the buttonlistener
         leftButton.setOnAction(new ButtonListener());
         rightButton.setOnAction(new ButtonListener());
         bottomButton.setOnAction(new ButtonListener());   
      } 
      ...
      public void draw() {//draws the circles
         ...
         if (ballIsVisible) {//draw the ball if its supposed to be visible
      ...
      public void setTopButtonVisible(boolean isVisible) {
         topIsVisible = isVisible;// set the buttons to the correct visiblity
      ...
      public class ButtonListener implements EventHandler<ActionEvent> {
         ...
         public void handle(ActionEvent e) {
            ...
            click(direction, iPos, jPos); //send the direction to the click method
         }
